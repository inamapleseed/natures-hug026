...
class ControllerCatalogGallimage extends Controller {
	...
	protected function getForm() {
		...
		if (isset($this->request->post['product_category'])) {
			...
			$categories = array();
			// $results = $this->model_catalog_category->getGallCategories2($this->request->get['gallimage_id']);
			// foreach ($results as $result) {
		...
		foreach ($categories as $category_id) {
			$category_info = $this->model_catalog_category->getGallCategory($category_id);
			if ($category_info) {
				$data['product_categories'][] = array(
