	...
	public function getProducts() {
		...
		foreach ($cart_query->rows as $cart) {
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				$product_categories_list = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_category where product_id='".(int)$cart['product_id']."'");

				foreach ($product_categories_list->rows as $procate) {
					$pro_cate[] = array(
						'product_category_id' => $procate['category_id'],
					);
				}
	...
	public function getTotal2() {
		...
		foreach ($this->getProducts() as $product) {
			if(isset($product['salecombinationquantity'])) {
	      	...
	      	//$total = $product['product_category'];
	      	foreach($product['product_category'] as $pc)
	      	{
	      	    if($pc['product_category_id'] == '7')
	      	    {
	      	        $total += 0;
	      	    }
	      	    else
